...
//    }
    ...
    public void init() {
        viewItems = new ArrayList<com.jj.drag.Pointer>(0);

        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.marker_film);
        com.jj.drag.Pointer p = new com.jj.drag.Pointer(context, bitmap, 10, 10);
        p.txt = "艺术影厅";

        Bitmap bitmapTip = BitmapFactory.decodeResource(context.getResources(), R.drawable.tip_frame);
        p.tip = new com.jj.drag.Pointer(context, bitmapTip, (p.immutableX + p.width)-p.width/2-bitmapTip.getWidth()/2, p.immutableY - p.height);
        viewItems.add(p);
