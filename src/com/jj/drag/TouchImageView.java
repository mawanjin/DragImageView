...
//                savedMatrix.set(matrix);
    ...
    private void Onclick(float x, float y) {
        ...
        for (Pointer pointer1 : viewItems) {
            ...
            if (pointer1.checkRange(a[0], a[1])) {
                ...
                invalidate();
                if (eventDispatcher != null) eventDispatcher.onClick(pointer1);
                break;
            }
            //检查tip点击事件
    ...
    private float[] checkBounds(float _offsetX, float _offsetY) {
        ...
        if ((x2 + _offsetX + marginLeft) <= widthScreen && a[0] < 0) a[0] = 0;
        //下向拖动
        if ((y2 > 0 && a[1] > 0)) a[1] = 0;
        //向上拖动
        if ((y3 < heightScreen && a[1] < 0)) a[1] = 0;
        return a;
    }
